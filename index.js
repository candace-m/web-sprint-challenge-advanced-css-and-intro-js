...
function getArtistByIndex(artistsArray, index) {
    /* code here */
    return `The artist at index ${index} is ${artistsArray[index].name}`
  }
...
function get20s(data){
  ...
  for (let i = 0; i < data.length; i++) {
    let tempArray2 = data[i].years.split(" - ");
    tempNum[0] = parseInt(tempArray2[0], 10);
    tempNum[1] = parseInt(tempArray2[1], 10);
    if (tempNum[0] >= 1900 && tempNum[0] <= 2000) {
      tempArray1.push(data[i].name);
    } else if(tempNum[1] >= 1900 && tempNum[1] <= 2000){
      tempArray1.push(data[i].name);
    }
  }
  return tempArray1;
...
*/
function removeArtist(data, num) {
    /* code here */
    data.splice(num,1)
    return data.length;
  }
...
let candace = [
    ...
    "bio": "Lots of important stuff. Lots of important stuff. Lots of important stuff",
    "wikipedia": "http://en.wikipedia.org/wiki/candace_morris",
    "paintings": 10000000
  }
]
...
console.log("Task #6",addArtist(candace))
...
function lotsOfArt(numArt){
  ...
  for(let i=0; i < numArt.length; i++) {
    if(numArt[i].paintings > 100) {
      tempArray.push(numArt[i].name)
    }
  }
  return tempArray;
}

console.log("Tasks # 7", lotsOfArt(artists));
